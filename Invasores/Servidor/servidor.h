...
typedef struct {
	int x, y, vida, taxaDisparo, velMovimento, nMovs;			//maior a taxa de disparo, mais movimentos terá de fazer antes de disparar	//nMovs -> Contador de movimentos p/ taxa de disparo
...
typedef struct {
	int dimX, dimY, nNavesDef, nNavesInvBasica, nNavesInvEsquiva,comecaJogo;
	Tiro tbp[50];		
	NaveDefensora navesDefensoras[2];									//Substituir por nMaxNavesDef num futuro próximo
	NaveInvasora navesInvasoras[50];									//Substituir por nMaxNavesInv num futuro próximo
